...
begin
  ...
  def relocate_vm(vm)
    ...
    #$evm.log(:info, "Our Server role is #{our_server_role}")
    #
    ...
    vm.ems_cluster.hosts.each do |this_host|      # <-- Two levels of Association
      ...
      this_host.vms.each do |this_vm|             # <-- Association
        $evm.log(:info, "Iterating host #{this_host}, found vm #{this_vm}")
        if this_vm.tags(:vm_role).first == our_server_role
          host_invalid = true
          $evm.log(:info, "found tag #{our_server_role}")
          break
        end
      end
  ...
  vm.host.vms.each do |this_vm|             # <-- Two levels of Association
    ...
    if this_vm.tags(:vm_role).first == "master"
      $evm.log(:info, "VM #{this_vm.name} - its master")
    elsif this_vm.tags(:vm_role).first == "secondary"
      $evm.log(:info, "VM #{this_vm.name} - its secondary")
    else
      next
    end
    
    #next if this_vm.tags(:vm_role).first != "secondary"
    if this_vm.tags(:vm_role).first != our_server_role      
      anti_role = this_vm.tags(:vm_role).first
      $evm.log(:info, "VM #{this_vm.name} has anti_affinity vm_role tag of: #{anti_role}, taking remedial action")
